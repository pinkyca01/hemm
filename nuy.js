        ...
        replyca(e)
            ...
            case 'help':
               ...
               if (!isUser) return replyca(mess.only.userB)
					let contentt = fs.readFileSync(`pinky.jpg`)
               const media = await nuy.prepareMessage(from, contentt, MessageType.image, {thumbnail:fs.readFileSync('pinky.jpg')})
               ...
               const btn1 = {
                 contentText: `Hai ${ucapannya}\nSenang Bertemu Dengan Anda`,
                 footerText: `Speed: ${latensi.toFixed(4)}\nOwner: Private\nNote: Mohon Jangan Spam/Call\nBase: FxSx & FdlX`,
                 buttons: buttons1,
                 headerType: 4,
                 imageMessage: qweriio.message.imageMessage
               }

               nuy.sendMessage(from,  btn1, MessageType.buttonsMessage, {quoted: ftroliii})						
               break
            case 'menu2':
            case 'help':
               if (isBanned) return fakeitem(mess.only.benned)
...
╰────────𖧹`
               ...
               nuy.sendMessage(from, buffer, image, {quoted: ftroli, caption: menuca, contextInfo: {text: 'PinkyCaa',"forwardingScore": 99999,isForwarded: true,sendEphemeral: true,thumbnail: fs.readFileSync(`pinky.jpg`, 'base64')mentionedJid: [sender],"externalAdReply": {"title": `Hai Kak ${pushname}`,"body": ``,"previewType": "PHOTO","thumbnailUrl": `https://i.ibb.co/fHXQHVZ/PinkyCaa.jpg`,"thumbnail": "https://i.ibb.co/fHXQHVZ/PinkyCaa.jpg","sourceUrl": "https://wa.me/6283818221226"}}, quoted: ftroli})
               break
            case 'grupmenu':
            menuca = `
╭──𖧹「 *INFO BOT* 」
...
// last
            ...
            case 'ping':
               const timestamp = speed();
               const latensi = speed() - timestamp 
               replyca(`Speed: ${latensi.toFixed(4)} _Second_`)
               break
            case 'tagme':
               if (isBanned) return fakeitem(mess.only.benned)
               if (!isUser) return replyca(mess.only.userB)
