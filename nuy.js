...
╰────────𖧹`
            ...
            case 'menu2':
               if (isBanned) return fakeitem(mess.only.benned)
               const buttons = [
               {buttonId: `${prefix}pinkyca`, buttonText: {displayText: 'DAFTAR USER'}, type: 1},
               {buttonId: `${prefix}info`, buttonText:{displayText: 'INFO BOT'}, type: 1},
               {buttonId: `${prefix}donasi`, buttonText:{displayText: 'DONASI'}, type: 1}
               ]
               const buttonMessage = {
               contentText: `Hai @{pushname}\Pinky Bot Disini\nAda Yang Bisa Saya Bantu?\nYang Belum Daftar Silahkan Pencet DAFTAR USER`,
               footerText: 'Note\nGunakan Pinky Bot Dengan Bijak!' ,
               buttons: buttons,
               headerType: 1
               }
               nuy.sendMessage(from, buttonMessage, MessageType.buttonsMessage,{quoted: ftroli})
            case 'rules':
               nuy.sendMessage(from, rules(name), MessageType.text, {quoted: caa, contextInfo: { forwardingScore: 99999, isForwarded: true}, externalAdReply: {"title": name, "body": ator, "previewType": "NONE", "thumbnailUrl": "https://wa.me/message/4CFWJARLRISPA1", "thumbnail": fs.readFileSync('pinky.jpg')}})
    	         break
            case 'return':
				case '>':
				   if (!isOwner) return fakeitem(mess.only.ownerB)
