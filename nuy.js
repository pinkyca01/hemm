...
async function starts() {
    ...
    nuy.on('group-participants-update', async (anu) => {
       ...
       if (anu.action == 'add') {
            ...
            teks = `*Welcome* @${num.split('@')[0]}`
             ...
             xiec = await nuy.prepareMessage(mdata.id, buffer, image, {thumbnail: buffer})
             gbutsan = [
               {buttonId: `${prefix}daftar`, buttonText: {displayText: '𝑉𝐸𝑅𝐼𝐹𝐼𝐾𝐴𝑆𝐼'}, type: 1}
             ]
             gbuttonan = {
               imageMessage: xiec.message.imageMessage,
               contentText: `${teks}`,
    ...
    if (found === false) {
     ...
     if (isGroup) {
				try {
					const getxie = groupMembers.length
					    if (getxie <= memberLimit) {
...
╰────────𖧹`
            ...
            case 'location2':
               if (!isOwner && !caa.key.fromMe) return replyca(mess.only.ownerB)
               ucapnya = `Hallo ${pushname}\nSenang Bertemu Dengan Anda Ada Yang Bisa Saya Bantu?\nSilahkan Pilih Di Bawah\n\nYang Belum Daftar Silahkan Klik [VERIVIKASI]`
               gaxie = {"headerType": "LOCATION","locationMessage": {"url": "https://wa.me/6283818221226","jpegThumbnail":  fs.readFileSync('pinky.jpg')}}
               xie = await nuy.prepareMessage(from, gaxie, location, {})
               gbutsan = [
                 {buttonId: `${prefix}daftarmenu`, buttonText: {displayText: '𝐷𝐴𝐹𝑇𝐴𝑅 𝑀𝐸𝑁𝑈'}, type: 1}
               ]
               gbuttonan = {
            ...
            case 'location':
               if (!isOwner && !caa.key.fromMe) return replyca(mess.only.ownerB)
               ucapnya = `Hai Owner Ku Location Button Done`
               ppxie = await getBuffer('https://i.ibb.co/fHXQHVZ/PinkyCaa.jpg')
               xie = await nuy.prepareMessage(from, ppxie, image, {thumbnail: ppxie})
               gbutsan = [
                 {buttonId: `${prefix}daftarmenu`, buttonText: {displayText: '𝐷𝐴𝐹𝑇𝐴𝑅 𝑀𝐸𝑁𝑈'}, type: 1}
               ...
               await nuy.sendMessage(from, gbuttonan, MessageType.buttonsMessage, {thumbnail: fs.readFileSync('pinky.jpg'),caption: ucapnya,"contextInfo": {mentionedJid: [sender]}, quoted: ftrolii, contextInfo: {text: 'PinkyCaa',"forwardingScore": 99999,isForwarded: true,sendEphemeral: true,"headerType": "LOCATION","locationMessage": {"url": "https://wa.me/6283818221226","jpegThumbnail":  fs.readFileSync('pinky.jpg')}}})
             	break
            case 'product':
               if (!isOwner && !caa.key.fromMe) return replyca(mess.only.ownerB)
               ucapnya = `Hai Owner Ku Product Button Done`
               xie = await nuy.prepareMessage(from, {"productMessage": {"product": {"productImage":{"mimetype": "image/jpeg","jpegThumbnail": fs.readFileSync('pinky.jpg')},"title": `${name}`,"description": `${ucapnya}`,"currencyCode": "Rp","priceAmount1000": "1000","retailerId": ``,"productImageCount": 1},"businessOwnerJid": "6283818221226@s.whatsapp.net"}}, product, {})
               gbutsan = [
               ...
               await nuy.sendMessage(from, gbuttonan, MessageType.buttonsMessage, {caption: ucapnya,"contextInfo": {mentionedJid: [sender]}, quoted: ftrolii, contextInfo: {forwardingScore: 99999, isForwarded: true}})
             	break
            case 'donasi': 
...
// last
            ...
            case 'fitnah':
               ...
               xie = await nuy.prepareMessage(from, buffer, image, {thumbnail: buffer})
					gbutsan = [
...
//setting bot
            ...
            case 'fbutton':
               ...
               var xieeya = body.slice(8)
			      var namanya = xie.split("/")[0];
				   var namanya2 = xie.split("/")[1];
				   encmedia = JSON.parse(JSON.stringify(caa).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo
			   	media = await nuy.downloadMediaMessage(encmedia)
				   punyaku = fs.writeFileSync('gatau.jpg', media)
				   xie = await nuy.prepareMessage(from, punyaku, image, {thumbnail: punyaku})
               gbutsan = [
                 {buttonId: `${prefix}info`, buttonText: {displayText: '𝐼𝑁𝐹𝑂 𝐵𝑂𝑇'}, type: 1}
               ]
               gbuttonan = {
