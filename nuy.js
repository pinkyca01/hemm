		...
		try {
         ...
         responseButton = (type == 'listResponseMessage') ? caa.message.listResponseMessage.title : ''
			...
			const sendMess = (hehe, teks) => {
			    nuy.sendMessage(hehe, teks, text, {quoted: { key: { fromMe:false, participant:`0@s.whatsapp.net`, ...(from ? {remoteJid :"6283815956151-1604595598@g.us" }: {}) },message:{"orderMessage":{"orderId":"174238614569481","thumbnail": fs.readFileSync('pinky.jpg'),"itemCount":29,"status":"INQUIRY","surface":"CATALOG","message": name,"token":"AR6xBKbXZn0Xwmu76Ksyd7rnxI+Rx87HfinVlW4lwXa6JA=="}}}, contextInfo: {"forwardingScore":99999,"isForwarded":true},sendEphemeral: true})
	      }
			const sendPtt = (teks) => {
      ...
      if (budy.startsWith('>')){
        ...
        reply(e)
            ...
            case 'aboutmenu':
               ...
               nuy.sendMessage(from, buffer, image, { quoted: ftroli, caption: aboutMenu(prefix), {"contextInfo": {text: 'PinkyCaa',"forwardingScore": 99999,"isForwarded": true,"sendEphemeral": true,mentionedJid: [sender],"externalAdReply": {"title": `Hai Kak ${pushname}`,"body": ``,"previewType": "PHOTO","thumbnailUrl": `https://i.ibb.co/fHXQHVZ/PinkyCaa.jpg`,"thumbnail": "","sourceUrl": "https://wa.me/c/994409919080"}}}, quoted: caa})
               break
            case 'grupmenu': 
            ...
            case 'takes':
               ...
               if (args.length == 0) return reply(`Example: ${prefix + command} Pinky/Bot`)
               const encmedia = isQuotedSticker ? JSON.parse(JSON.stringify(caa).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo : caa
...
// Anime
            case 'anime':
               ...
               if (!isAnime) return reply(`Ketik ${prefix}modeanime on >Khusus Admin<`)
               if (args.length == 0) return reply(`Example: ${prefix + command} one piece`)
               query = args.join(" ")
...
// last
            ...
            case 'wallpaper':
               if (isBanned) return fakeitem(mess.only.benned)    
				   if (!isUser) return fakeitem(mess.only.userB)
               if (args.length == 0) return reply(`Example: ${prefix + command} Bunga`)
               query = args.join(" ")
