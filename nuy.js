...
// last
            ...
            case 'wasted':
               if (isBanned) return fakeitem(mess.only.benned)   
				   if (!isUser) return fakeitem(mess.only.userB)
               url = args[0]
               buffer = await getBuffer(`https://api.lolhuman.xyz/api/editor/wasted?apikey=${LolApi}&img=${url}`)
               nuy.sendMessage(from, buffer, image, {quoted: caa})
               break
            case 'smoji':
               if (isBanned) return fakeitem(mess.only.benned)   
               ...
               buffer = await getBuffer(`https://api.lolhuman.xyz/api/smoji/${emoji}?apikey=${LolApi}`)
