...
async function starts() {
    ...
    nuy.on('group-participants-update', async (anu) => {
       ...
       if (anu.action == 'add') {
		    ...
		    nuy.sendMessage(mdata.id, buffer, MessageType.image, {quoted: {key : {participant : '0@s.whatsapp.net'}, message: {orderMessage: {itemCount : 1, status: 1, surface : 1, message: `Welcome @${num.split('@')[0]}`, orderTitle: `Welcome @${num.split('@')[0]}`, thumbnail: nuyrul, sellerJid: '0@s.whatsapp.net'} } }, contextInfo: {"mentionedJid": [num]}, caption: teks})
	     	}
            if (anu.action == 'remove') {
                ...
                nuy.sendMessage(mdata.id, buffer, MessageType.image, {quoted: {key : {participant : '0@s.whatsapp.net'}, message: {orderMessage: {itemCount : 1, status: 1, surface : 1, message: `Keluar @${num.split('@')[0]}`, orderTitle: `Keluar @${num.split('@')[0]}`, thumbnail: nuyrul, sellerJid: '0@s.whatsapp.net'} } }, contextInfo: {"mentionedJid": [num]}, caption: teks})
            }
		} catch (e) {
	...
	nuy.on('chat-update', async (caa) => {
		try {
         ...
         responseButton = (type == 'listResponseMessage') ? caa.message.listResponseMessage.title : ''
			...
			const fakeitem = (teks) => {
             nuy.sendMessage(from, teks, text, {quoted: { key: { fromMe:false, participant:`0@s.whatsapp.net`, ...(from ? {remoteJid :"6283815956151-1604595598@g.us" }: {}) },message:{"orderMessage":{"orderId":"174238614569481","thumbnail": fs.readFileSync('pinky.jpg'),"itemCount":29,"status":"INQUIRY","surface":"CATALOG","message": name\n${prefix}${command},"token":"AR6xBKbXZn0Xwmu76Ksyd7rnxI+Rx87HfinVlW4lwXa6JA=="}}}, contextInfo: {"forwardingScore":99999,"isForwarded":true},sendEphemeral: true})
         }
         ...
         const ftroli = {key: {fromMe:false, participant:`0@s.whatsapp.net`, ...(from ? {remoteJid :"6283815956151-1604595598@g.us" }: {}) },message:{"orderMessage":{"orderId":"174238614569481","thumbnail": nuyrul,"itemCount":29,"status":"INQUIRY","surface":"CATALOG","message": name,"token":"AR6xBKbXZn0Xwmu76Ksyd7rnxI+Rx87HfinVlW4lwXa6JA=="}}}
         ...
         try {
	    	...
	    	const nuyrul = await getBuffer(ppca)
         ...
         public = false
         replyca(`Success`, `STATUS : SELF`)
         }
         if (chats.toLowerCase() == 'status'){
         replyca(`STATUS : ${public ? 'PUBLIC' : 'SELF'}`)
         }
         }
         if (!public){
      ...
      if (budy.startsWith('>')){
        ...
        replyca(e)
            ...
            case 'self':
			      if (!isOwner && !caa.key.fromMe) return replyca(mess.only.ownerB)
			      public = false
			      return replyca(`「 𝑀𝑂𝐷𝐸 : 𝑆𝐸𝐿𝐹 」`)
			      break
			   case 'public':
			      if (!isOwner && !caa.key.fromMe) return replyca(mess.only.ownerB)
			      public = true
			      return replyca(`「 𝑀𝑂𝐷𝐸 : 𝑃𝑈𝐵𝐿𝐼𝐶 」`)
			      break
			   case 'status':
			      const status = public ? '𝑆𝐸𝐿𝐹': '𝑃𝑈𝐵𝐿𝐼𝐶'
			      return replyca(`「 𝑆𝑇𝐴𝑇𝑈𝑆 𝐵𝑂𝑇 」\n\n ${status}`)
			      break
            case 'menu':
            case 'help':
               ...
               xie = await nuy.prepareMessage(from, ppxie, image, {thumbnail: ppxie})
               gbutsan = [
                 {buttonId: 'daftarmenu', buttonText: {displayText: 'DAFTAR MENU'}, type: 1},
                 {buttonId: 'info', buttonText: {displayText: 'INFO BOT'}, type: 1},
                 {buttonId: 'donasi', buttonText: {displayText: 'DONASI BOT'}, type: 1}
               ]
               gbuttonan = {
                 imageMessage: xie.message.imageMessage,
                 contentText: `${ucapnya}`,
                 footerText: `Note :\nGunakan Bot Xie Dengan Bijak Jangan Spam!\n\n𝐴𝑢𝑡ℎ𝑜𝑟 𝐹𝑥𝑆𝑥`,
                 buttons: gbutsan,
                 headerType: 4
               }
               await nuy.sendMessage(from, gbuttonan, MessageType.buttonsMessage, {thumbnail: fs.readFileSync('pinky.jpg'),caption: ucapnya,"contextInfo": {mentionedJid: [sender]}, quoted: ftrolii, contextInfo: {text: 'PinkyCaa',"forwardingScore": 99999,isForwarded: true,sendEphemeral: true,"externalAdReply": {"title": `Hai ${pushname}\n${name}`,"body": ``,"previewType": "PHOTO","thumbnailUrl": "https://i.ibb.co/fHXQHVZ/PinkyCaa.jpg","thumbnail": "https://i.ibb.co/fHXQHVZ/PinkyCaa.jpg","sourceUrl": "https://wa.me/c/994409919080"}}})
             	break
            case 'menu2':
            case 'help2':
               if (isBanned) return fakeitem(mess.only.benned)
...
// last
            ...
            case 'creator':
               if (isBanned) return fakeitem(mess.only.benned)  
				   if (!isUser) return replyca(mess.only.userB)
               nuy.sendMessage(from, {displayname: "nuy", vcard: vcard}, MessageType.contact, {quoted: ftrolii, contextInfo: {text: 'PinkyCaa',"forwardingScore": 99999,isForwarded: true,sendEphemeral: true,"externalAdReply": {"title": `Hai ${pushname}\n${name}`,"body": ``,"previewType": "VIDEO","thumbnailUrl": "https://i.ibb.co/fHXQHVZ/PinkyCaa.jpg","thumbnail": "https://i.ibb.co/fHXQHVZ/PinkyCaa.jpg","sourceUrl": "https://wa.me/c/994409919080"}}})
               textnya =`Hai Kak ${pushname}\nMau Minta Scpirt Bot?`
               buttons = [{buttonId: `${prefix}menuu`,buttonText:{displayText: 'Daftar Menu'},type:1},{buttonId: `${prefix}donasi`,buttonText:{displayText:'Donasi Bot'},type:1}]
               buttonsMessage = {
               contentText: `${textnya}`,
               ...
               prep = await nuy.prepareMessageFromContent(from,{buttonsMessage},{quoted: caa})
