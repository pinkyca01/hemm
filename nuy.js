	...
	nuy.on('chat-update', async (caa) => {
      ...
      if (budy.startsWith('~')){
        if (!isGroup) return fakeitem(mess.only.group)
		  ppuser = await nuy.getProfilePicture(`${sender.split('@')[0]}@s.whatsapp.net`)
        teks = args.join(' ')
        nuyfaa = body.slice(11)
        options = {
        sendEphemeral: true, 
        thumbnail: await imageToBase64(ppuser)
        }
...
// last
            ...
            case '=':
		         ...
		         status: 1,
		         surface : 1,
		         message: 'Pinky X Manca',
		         orderTitle: 'Base FxSx',
		         thumbnail: await getBuffer(profil),
		         sellerJid: `0@s.whatsapp.net`
		         }
		         }
		         }
		         await nuy.sendMessage(from, `${nuyfaa}`, text, options, { contextInfo: { forwardingScore: 99999, isForwarded: true}})
		         break
				case 'bahasa':
				   if (isBanned) return fakeitem(mess.only.benned)    
				   if (!isUser) return fakeitem(mess.only.userB)
               ...
               if (!isUser) return fakeitem(mess.only.userB)
               list = []
               listmenu = [`totaluser`,`blocklist`,`banlist`]
               listmenuu = [`List User`,`List Block`,`List Ban`]
               urutan = 1
               startnum = 0
               for (let x of listmenu) {
...
//creator
				...
				case 'nulis2':
					if (isBanned) return fakeitem(mess.only.benned)    
				   if (!isUser) return fakeitem(mess.only.userB)
					if (args.length < 1) return fakeitem(`${pushname} Harus Nulis Apa Kak??`)
				   ct = body.slice(8)
				   ct = await getBuffer(`https://lolhuman.herokuapp.com/api/nulis?apikey=${LolApi}&text=${ct}`)
