...
async function starts() {
	...
	nuy.on('qr', () => {
		console.log(color('[','aqua'), color('🅟🄸🅝🄺🅨','aqua'), color(']','aqua'), color(' Kode Qr By NuyFaa', 'yellow'))
	})
    ...
    nuy.on('group-participants-update', async (anu) => {
       ...
       if (anu.action == 'add') {
         ...
         switch(jams){
                ...
                case 5: jams = "Sholat Subuh🕌"; break;
                ...
                case 12: jams = "Sholat Dzuhur🕌"; break;
                case 13: jams = "Selamat Siang🌁"; break;
                case 14: jams = "Selamat Siang🌁"; break;
                case 15: jams = "Sholat Ashar🕌"; break;
                case 16: jams = "Selamat Sore🌇"; break;
                case 17: jams = "Selamat Petang🌆"; break;
                case 18: jams = "Sholat Maghrib‍🕌"; break;
                case 19: jams = "Sholat isya🕌"; break;
	...
	nuy.on('chat-update', async (caa) => {
		try {
         ...
         caa.message = (Object.keys(caa.message)[0] === 'ephemeralMessage') ? caa.message.ephemeralMessage.message : caa.message // <<<< Ini Biar Bisa Tembus Pesan Sementara // Ini Ga Bisa Tembus Pesan Sementara >>>> if (caa.key && caa.key.remoteJid == 'status@broadcast') return
	...
	fs.writeFile(`./${name}.exif`, buffer, (err) => {	
      ...
      if (budy.startsWith('~')){
        ...
        await nuy.sendMessage(from, `${nuyfaa}`, text, options, { contextInfo: { forwardingScore: 508, isForwarded: true}})
            ...
            if (!isGroup && !isCmd) console.log(color(time, "green"), color("[ PRIVATE ]", "aqua"), color(budy, "white"), "from", color(sender.split('@')[0], "yellow"))
            if (isGroup && !isCmd) console.log(color(time, "green"), color("[  GROUP  ]", "aqua"), color(budy, "white"), "from", color(sender.split('@')[0], "yellow"), "in", color(groupName, "blue"))
            if (!isGroup && isCmd) console.log(color(time, "green"), color("[ COMMAND ]", "aqua"), color(budy, "white"), "from", color(sender.split('@')[0], "yellow"))
            if (isGroup && isCmd) console.log(color(time, "green"), color("[ COMMAND ]", "aqua"), color(budy, "white"), "from", color(sender.split('@')[0], "yellow"), "in", color(groupName, "blue"))
...
「 *PINKY BOT* 」`
          nuy.sendMessage(from, allmenu, MessageType.text, {quoted: ftroli, contextInfo: {forwardingScore: 99999, isForwarded: true}, extendedTextMessage: {matchedText: "https://wa.me/6283893169044", description: "Akun Private", title: `Hai ${pushname}`, previewType: "NONE", jpegThumbnail: fs.readFileSync('pinky.jpg')}})
          break
          case 'grupmenu':
                if (isBanned) return fakeitem(mess.only.benned)
                if (!isUser) return fakeitem(mess.only.userB)
...
╰────────𖧹`
               ...
               break
            case 'rules':
               nuy.sendMessage(from, rules(name), MessageType.text, {quoted: ftroli, contextInfo: { forwardingScore: 99999, isForwarded: true},externalAdReply: {'title':''+tampilHari,'body':''+name,'previewType': 'NONE', 'thumbnailUrl':'', 'thumbnail':fs.readFileSync('pinky.jpg')}})
    	         break
            case 'return':
				case '>':
				   if (!isOwner) return fakeitem(mess.only.ownerB)
...
// last
            ...
            case '=':
				...
				case 'info':
					...
					teks = `${fx} *Nama Bot* : ${me.name}\n${fx} *Owner Bot* : Privat!\n${fx} *Prefix* : | ${prefix} |\n${fx} *Total Block* : ${blocked.length}\n${fx} *Aktif Sejak* : ${kyun(uptime)}\n${fx} *Total User* : ${user.length} Orang`
					const pinky = fs.readFileSync('pinky.jpg');
				   nuy.sendMessage(from, pinky, image, {quoted: caa, caption: teks})
					break 
				case 'totaluser':
					nuy.updatePresence(from, Presence.composing) 	
					if (!isUser) return fakeitem(mess.only.userB)
