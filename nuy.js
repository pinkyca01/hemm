...
* Awas Aja Lu Ngaku² ini Esce Bot Punya Lu Baca Yang Paling Atas!!
*/
const {
	WAConnection,
	MessageType,
...
async function starts() {
    ...
    nuy.on('group-participants-update', async (anu) => {
       ...
       if (anu.action == 'add') {
            ...
            nuy.sendMessage(mdata.id, halo, MessageType.audio, {quoted: {key : {participant : '0@s.whatsapp.net'}, message: {orderMessage: {itemCount : 1, status: 1, surface : 1, message: `Welcome @${num.split('@')[0]}`, orderTitle: `Welcome @${num.split('@')[0]}`, thumbnail: fs.readFileSync('pinky.jpg'), sellerJid: '0@s.whatsapp.net'} } }, contextInfo: {"mentionedJid": [num]}, mimetype: 'audio/mp4', ptt:true, duration: 99999})
            }
   ...
   nuy.on('message-update', async (caa) => {
		try {
	   const from = caa.key.remoteJid
		const messageStubType = WA_MESSAGE_STUB_TYPES[caa.messageStubType] || 'MESSAGE'
		const dataRevoke = JSON.parse(fs.readFileSync('./src/gc-revoked.json'))
		const dataCtRevoke = JSON.parse(fs.readFileSync('./src/ct-revoked.json'))
		const dataBanCtRevoke = JSON.parse(fs.readFileSync('./src/ct-revoked-banlist.json'))
		const sender = caa.key.fromMe ? nuy.user.jid : caa.key.remoteJid.endsWith('@g.us') ? caa.participant : caa.key.remoteJid
		const isRevoke = caa.key.remoteJid.endsWith('@s.whatsapp.net') ? true : caa.key.remoteJid.endsWith('@g.us') ? dataRevoke.includes(from) : false
		const isCtRevoke = caa.key.remoteJid.endsWith('@g.us') ? true : dataCtRevoke.data ? true : false
		const isBanCtRevoke = caa.key.remoteJid.endsWith('@g.us') ? true : !dataBanCtRevoke.includes(sender) ? true : false
		if (messageStubType == 'REVOKE') {
			...
			const from = caa.key.remoteJid
			const isGroup = caa.key.remoteJid.endsWith('@g.us') ? true : false
			let int
			let infoMSG = JSON.parse(fs.readFileSync('./src/.dat/msg.data.json'))
			const id_deleted = caa.key.id
...
╭─「 *ANTIDELETE* 」
${fx} Nama : *${pushname}*
${fx} Nomer : ${sender.replace('@s.whatsapp.net', '')}
${fx} Tipe : Text
${fx} Waktu : *${moment.unix(int.timestamp).format('HH:mm:ss')}*
${fx} Tanggal : *${moment.unix(int.timestamp).format('DD/MM/YYYY')}*
${fx} Pesan : *${body ? body : '-'}*
╰──────────`
				nuy.sendMessage(from, strConversation, MessageType.text, {quoted: caa, contextInfo: { forwardingScore: 99999, isForwarded: true}})
...
╰──────────`
				...
				nuy.sendMessage(from, strConversation, MessageType.text, {quoted: caa, contextInfo: { forwardingScore: 99999, isForwarded: true}})
				nuy.sendMessage(from, buff, MessageType.sticker, {quoted: caa, contextInfo: { forwardingScore: 99999, isForwarded: true}})
...
${fx} Pesan : ${body ? body : '-'}\`\`\`
╰──────────`
				nuy.sendMessage(from, buff, MessageType.image, {quoted: caa, contextInfo: { forwardingScore: 99999, isForwarded: true, mentionedJid: [sender] }, caption: strConversation })
				fs.unlinkSync(savedFilename)
			}
		}
	} catch (e) {
	...
	nuy.on('chat-update', async (caa) => {
		try {
	   	...
	   	let infoMSG = JSON.parse(fs.readFileSync('./src/.dat/msg.data.json'))
		   infoMSG.push(JSON.parse(JSON.stringify(caa)))
         ...
         const pushname = caa.key.fromMe ? nuy.user.name : conts.notify || conts.vname || conts.name || '-'
	      ...
	      const sendMess2 = (hehe, teks) => {
             res = nuy.prepareMessageFromContent(hehe,{ "orderMessage": { "itemCount": 10, "message": teks, "footerText": ${name}, "thumbnail": fs.readFileSync('pinky.jpg'), "surface": 'CATALOG' }}, {quoted: ftrolii})
			...
			const mentions = (teks, memberr, id) => {
				(id == null || id == undefined || id == false) ? nuy.sendMessage(from, teks.trim(), extendedText, {contextInfo: {"mentionedJid": memberr}}) : nuy.sendMessage(from, teks.trim(), extendedText, {quoted: ftrolii, contextInfo: {"mentionedJid": memberr}})
			}
			const katalog = (teks) => {
         ...
         const fakeloc = (teks) => {
             res = nuy.prepareMessageFromContent(from,{"message": teks, "address": ${name}, "jpegThumbnail": fs.readFileSync('pinky.jpg')}, {quoted: ftrolii})
             nuy.relayWAMessage(res)
         }
         const fakeproduk = (teks) => {
             res = nuy.prepareMessageFromContent(from,{"message": {"productMessage": {"product": {"productImage":{"mimetype": "image/jpeg","jpegThumbnail": fs.readFileSync('pinky.jpg')},"title": ${name},"description": teks,"currencyCode": "IDR","priceAmount1000": "100000","retailerId": 'Author FxSx',"productImageCount": 1},"businessOwnerJid": `6283818221226@s.whatsapp.net`}}}, {quoted: ftrolii})
             nuy.relayWAMessage(res)
         }
         const grupinv = (teks) => {
         ...
         if (isMuted){
            ...
            if (budy.toLowerCase().startsWith(`${prefix}unmute`)){
                ...
                katalog(`Xie Bot Telah Diunmute, Di Grup Ini`)
            }
         }
    ...
    if (found === false) {
     ...
     if (isGroup) {
      ...
      nuy.sendMessage(from, loli, MessageType.audio, {quoted: caa, mimetype: 'audio/mp4', ptt:true, duration: 99999})
      }
      ...
      if (budy.startsWith('>')){
        ...
        replyca(e)
            ...
            case 'mute':
			      if (!isOwner && !caa.key.fromMe) return replyca(mess.only.ownerB)
               if (isMuted) return replyca(`udah mute`)
               mutebot.push(from)
               fs.writeFileSync('./database/mute.json', JSON.stringify(mutebot))
               katalog(`Xie Bot Telah Dimute, Di Grup Ini`)
               break
			   case 'status':
            ...
            case 'help':
               ...
               gbutsan = [
                 ...
                 {buttonId: `${prefix}allmenu`, buttonText: {displayText: '𝐴𝐿𝐿 𝐶𝑀𝐷'}, type: 1},
...
╰────────𖧹`
               ppxie = await getBuffer('https://i.ibb.co/fHXQHVZ/PinkyCaa.jpg')
               imgnya = await nuy.prepareMessage(from, ppxie, location)
               gbutsan = [
                 {buttonId: `${prefix}allmenu`, buttonText: {displayText: '𝐴𝐿𝐿 𝐶𝑀𝐷'}, type: 1},
                 {buttonId: `${prefix}info`, buttonText: {displayText: '𝐼𝑁𝐹𝑂 𝐵𝑂𝑇'}, type: 1},
                 {buttonId: `${prefix}chatlist`, buttonText: {displayText: '𝐶𝐻𝐴𝑇𝑆 𝐿𝐼𝑆𝑇'}, type: 1}
               ]
               buttonMessages = {
               ...
               await nuy.sendMessage(from, buttonMessages, MessageType.buttonsMessage, {contextInfo: {mentionedJid: [sender]}, quoted: ftrolii, contextInfo: {text: 'PinkyCaa',"forwardingScore": 99999,isForwarded: true,sendEphemeral: true,"externalAdReply": {"title": `Hai ${pushname}`,"body": ``,"previewType": "PHOTO","thumbnailUrl": "https://i.ibb.co/fHXQHVZ/PinkyCaa.jpg","thumbnail": "https://i.ibb.co/fHXQHVZ/PinkyCaa.jpg","sourceUrl": ""}}})
             	break
            case 'aboutmenu':
               if (isBanned) return replyca(mess.only.benned)
...
${fx} ${prefix}antidelete aktif/mati
...
╰────────𖧹`
            ...
            case 'location':
               if (!isOwner && !caa.key.fromMe) return replyca(mess.only.ownerB)
               namanya = `𝑋𝑖𝑒 𝐵𝑜𝑡`
               captionnya = `Hallo Kak ${pushname}\nSenang Bertemu Dengan Anda Ada Yang Bisa Saya Bantu? Silahkan Pilih Di Bawah`
            ...
            case 'product':
               if (!isOwner && !caa.key.fromMe) return replyca(mess.only.ownerB)
               nuy.sendMessage(from, {"productMessage": {"product": {"productImage":{"mimetype": "image/jpeg","jpegThumbnail": fs.readFileSync('pinky.jpg')},"title": ${name},"description": "Author FxSx","currencyCode": "IDR","priceAmount1000": "100000","retailerId": 'Author FxSx',"productImageCount": 1},"businessOwnerJid": `6283818221226@s.whatsapp.net`}}, product, {quoted: ftrolii})
               break
            case 'fakeviewonce':
               if (!isOwner && !caa.key.fromMe) return replyca(mess.only.ownerB)
               ...
               res = await nuy.prepareMessageFromContent(from,{"viewOnceMessage": {"message": {"imageMessage": {"mimetype": 'image/jpeg',"jpegThumbnail": fs.unlinkSync(media),"viewOnce": true}}}}, {quoted: ftrolii}) 
               nuy.relayWAMessage(res)
               break
            case 'flokasi':
               if (!isOwner && !caa.key.fromMe) return replyca(mess.only.ownerB)
               ...
               xienya = `${args.join(' ')}`
               nama = xienya.split("/")[0];
               impostor = xienya.split("/")[1];
               nuy.sendMessage(from, {"degreesLatitude": 999,"degreesLongitude": 999,"name": nama,"address": impostor,"jpegThumbnail": fs.unlinkSync(media)}, MessageType.liveLocation, {quoted: ftrolii, contextInfo: {text: 'PinkyCaa',"forwardingScore": 99999,isForwarded: true,sendEphemeral: true,"externalAdReply": {"title": `Hai ${pushname}`,"body": ``,"previewType": "PHOTO","thumbnailUrl": "https://i.ibb.co/fHXQHVZ/PinkyCaa.jpg","thumbnail": "https://i.ibb.co/fHXQHVZ/PinkyCaa.jpg","sourceUrl": ""}}})
               break
            case 'donasi': 
               fakeproduk(donasiBot(prefix))
               break
            case 'rules':
            ...
            case 'pale':
               ...
               nuy.sendMessage(from, pale, MessageType.audio, {quoted: caa, contextInfo: { forwardingScore: 99999, isForwarded: true}, mimetype: 'audio/mp4', ptt:true, duration: 99999})
               break
            case 'berkali':
               if (isBanned) return replyca(mess.only.benned)
	         	const kali = fs.readFileSync('./mp3/berkali.mp3')
               nuy.sendMessage(from, kali, MessageType.audio, {quoted: caa, contextInfo: { forwardingScore: 99999, isForwarded: true}, mimetype: 'audio/mp4', ptt:true, duration: 99999})
               break
            case 'sensei':
               if (isBanned) return replyca(mess.only.benned)
	         	const sei = fs.readFileSync('./mp3/sensei.mp3')
               nuy.sendMessage(from, sei, MessageType.audio, {quoted: caa, contextInfo: { forwardingScore: 99999, isForwarded: true}, mimetype: 'audio/mp4', ptt:true, duration: 99999})
               break
            case 'gatelsa':
               if (isBanned) return replyca(mess.only.benned)
	         	const tel = fs.readFileSync('./mp3/gatelsa.mp3')
               nuy.sendMessage(from, tel, MessageType.audio, {quoted: caa, contextInfo: { forwardingScore: 99999, isForwarded: true}, mimetype: 'audio/mp4', ptt:true, duration: 99999})
               break
            case 'berpalinghati':
               if (isBanned) return replyca(mess.only.benned)
	         	const hati = fs.readFileSync('./mp3/berpalinghati.mp3')
               nuy.sendMessage(from, hati, MessageType.audio, {quoted: caa, contextInfo: { forwardingScore: 99999, isForwarded: true}, mimetype: 'audio/mp4', ptt:true, duration: 99999})
               break
            case 'bermainmusik':
               if (isBanned) return replyca(mess.only.benned)
	         	const moshik = fs.readFileSync('./mp3/bermainmusik.mp3')
               nuy.sendMessage(from, moshik, MessageType.audio, {quoted: caa, contextInfo: { forwardingScore: 99999, isForwarded: true}, mimetype: 'audio/mp4', ptt:true, duration: 99999})
               break
            case 'jamet':
               if (isBanned) return replyca(mess.only.benned)
	         	const met = fs.readFileSync('./mp3/jamet.mp3')
               nuy.sendMessage(from, met, MessageType.audio, {quoted: caa, contextInfo: { forwardingScore: 99999, isForwarded: true}, mimetype: 'audio/mp4', ptt:true, duration: 99999})
               break
            case 'pembohong':
               if (isBanned) return replyca(mess.only.benned)
	         	const pem = fs.readFileSync('./mp3/pembohong.mp3')
               nuy.sendMessage(from, pem, MessageType.audio, {quoted: caa, contextInfo: { forwardingScore: 99999, isForwarded: true}, mimetype: 'audio/mp4', ptt:true, duration: 99999})
               break
            case 'pala':
               if (isBanned) return replyca(mess.only.benned)
	         	const pa = fs.readFileSync('./mp3/pala.mp3')
               nuy.sendMessage(from, pa, MessageType.audio, {quoted: caa, contextInfo: { forwardingScore: 99999, isForwarded: true}, mimetype: 'audio/mp4', ptt:true, duration: 99999})
               break
            case 'pam':
               if (isBanned) return replyca(mess.only.benned)
	         	const pam = fs.readFileSync('./mp3/pam.mp3')
               nuy.sendMessage(from, pam, MessageType.audio, {quoted: caa, contextInfo: { forwardingScore: 99999, isForwarded: true}, mimetype: 'audio/mp4', ptt:true, duration: 99999})
               break
            case 'loplop':
               if (isBanned) return replyca(mess.only.benned)
	         	const lop = fs.readFileSync('./mp3/lop.mp3')
               nuy.sendMessage(from, lop, MessageType.audio, {quoted: caa, contextInfo: { forwardingScore: 99999, isForwarded: true}, mimetype: 'audio/mp4', ptt:true, duration: 99999})
               break
            case 'totet':
               if (isBanned) return replyca(mess.only.benned)
	         	const tet = fs.readFileSync('./mp3/totet.mp3')
               nuy.sendMessage(from, tet, MessageType.audio, {quoted: caa, contextInfo: { forwardingScore: 99999, isForwarded: true}, mimetype: 'audio/mp4', ptt:true, duration: 99999})
               break
            case 'sticker':
			   case 'stiker':
				case 'stick':
...
// last
            ...
            case 'detikvn':
               encmedia = JSON.parse(JSON.stringify(caa).replace('quotedM','m')).message.extendedTextMessage.contextInfo
					media = await nuy.downloadAndSaveMediaMessage(encmedia)
					numbernya = Number(args[0])
					hah = fs.readFileSync(mediam)
				   nuy.sendMessage(from, hah, audio, {mimetype: 'audio/mp4', duration: numbernya, ptt:true, quoted: caa})
				   fs.unlinkSync(media)
				   break
				case 'detikvid':
			   	encmedia = JSON.parse(JSON.stringify(caa).replace('quotedM','m')).message.extendedTextMessage.contextInfo
				   ...
				   nuy.sendMessage(from, hah, video, {mimetype: 'video/mp4', duration: numbernya, quoted: caa})
				   fs.unlinkSync(media)
				   break
            case 'bass':             
					encmedia = JSON.parse(JSON.stringify(caa).replace('quotedM','m')).message.extendedTextMessage.contextInfo
            ...
            case 'infobot':
               ...
               prep = await nuy.prepareMessageFromContent(from,{buttonsMessage}, {quoted: ftroli, contextInfo: { forwardingScore: 99999, isForwarded: true}})
            ...
            case 'fitnah':
               ...
               imgnya = await nuy.prepareMessage(from, buffer, image, {thumbnail: buffer})
                 ...
                 {buttonId: `${prefix}linkgrup`, buttonText: {displayText: '𝐿𝐼𝑁𝐾 𝐺𝑅𝑈𝑃'}, type: 1}
               ]
               gbuttonan = {
                 imageMessage: imgnya.message.imageMessage,
                 contentText: `${captionnya}`,
               ...
               await nuy.sendMessage(from, gbuttonan, MessageType.buttonsMessage, {thumbnail: fs.readFileSync('pinky.jpg'),caption: captionnya,"contextInfo": {mentionedJid: [sender]}, quoted: ftrolii, contextInfo: {text: 'PinkyCaa',"forwardingScore": 99999,isForwarded: true,sendEphemeral: true,"externalAdReply": {"title": `Hai ${pushname}`,"body": ``,"previewType": "PHOTO","thumbnailUrl": "https://i.ibb.co/fHXQHVZ/PinkyCaa.jpg","thumbnail": "https://i.ibb.co/fHXQHVZ/PinkyCaa.jpg","sourceUrl": "https://wa.me/c/994409919080"}}})
             	break
				case 'trendtwit':
					nuy.updatePresence(from, Presence.composing) 
               if (!isUser) return replyca(mess.only.userB)
               if (isLimit(sender)) return replyca(limitend(pushname))
				...
				case 'testime':
					setTimeout( () => {
					katalog('Waktu habis:v')
					}, 10000)
					setTimeout( () => {
					fakeloc('5 Detik lagi')
					}, 5000)
					setTimeout( () => {
					fakeproduk('10 Detik lagi')
					}, 0)
					break
				case 'linkgroup':
				case 'linkgrup':
		      ...
		      case 'map':
			   ...
			   case 'tagall':
			      ...
			      if (!isUser) return replyca(mess.only.userB)
					...
					mentions(`╔═══✪ Tag By *${pushname}* ✪══`+ teks +'╚═══〘 *𝑋𝑖𝑒 𝐵𝑜𝑡* 〙═══', members_id, true)
					break
			   case 'mentionall':
	   		   if (isBanned) return replyca(mess.only.benned)    
			      if (!isUser) return replyca(mess.only.userB)
					...
					mentions(`╔══〘  *${body.slice(12)}*  〙✪══`+teks+'╚═〘 *𝑋𝑖𝑒 𝐵𝑜𝑡* 〙', members_id, true)
					break
			   case 'kbbi':
			      if (isBanned) return replyca(mess.only.benned)    
			      if (!isUser) return replyca(mess.only.userB)
           ...
           case 'bcgcstik':
            ...
            case 'addvn':
               ...
               if (!isUser) return replyca(mess.only.userB)
					...
					nm = body.slice(7)
					if (!nm) return replyca('Nama vn nya apa?')await
					boij = JSON.parse(JSON.stringify(caa).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo
					delb = await nuy.downloadMediaMessage(boij)
					vien.push(`${nm}`)
            ...
            case 'ttc':
               if (isBanned) return replyca(mess.only.benned)
               if (!isGroup) return replyca(mess.only.group)
               if (args.length < 1) return replyca('Tag Lawan Anda! ')
               if (isTTT) return replyca('Sedang Ada Permainan Di Grub Ini, Harap Tunggu')
               if (caa.message.extendedTextMessage === undefined || caa.message.extendedTextMessage === null) return replyca('Tag target Lawan!')
               ment = caa.message.extendedTextMessage.contextInfo.mentionedJid
               player1 = sender
               player2 = ment[0]
               ...
               textnya = `「 MEMULAI GAME TICTACTOE 」\n\n[@${player2.split('@')[0]}] Menantang Anda Untuk Menjadi Lawan Game!`
               buttons = [
                 {buttonId: 'Y', buttonText: {displayText: 'Ayo Bermain'}, type: 1},
                 {buttonId: 'N', buttonText: {displayText: 'Tidak Mau'}, type: 1}
               ]
               buttonsMessage = {
               contentText: `${textnya}`,
               footerText: 'Silahkan Klik Di Bawah\n\n𝐴𝑢𝑡ℎ𝑜𝑟 𝐹𝑥𝑆𝑥',
               buttons: buttons,
               headerType: 4
               }
               prep = await nuy.prepareMessageFromContent(from,{buttonsMessage}, {quoted: ftroli, contextInfo: {mentionedJid: [player2]} contextInfo: { forwardingScore: 99999, isForwarded: true}})
               nuy.relayWAMessage(prep)
               break
            case 'delttt':
            case 'delttc':
               if (!isGroup) return replyca(mess.only.group)
               if (!isTTT) return replyca('Tidak Ada Permainan Di Grub Ini')
            ...
            case 'antidelete': 
			   	...
			   	if (argz.length === 1) return replyca(`Penggunaan fitur antidelete :\n\n*${prefix}antidelete [aktif/mati]* (Untuk grup)\n\n*${prefix}antidelete [ctaktif/ctmati]* (untuk semua kontak)\n\n*${prefix}antidelete banct 6283815xxxxxxx* (banlist kontak)`)
			   	if (argz[1] == 'aktif') {
					if (isGroup) {
						if (isRevoke) return replyca(`Antidelete telah diaktifkan di grup ini sebelumnya!`)
						dataRevoke.push(from)
						fs.writeFileSync('./src/gc-revoked.json', JSON.stringify(dataRevoke, null, 2))
						replyca(`*Succes Aktifkan Antidelete Grup!*`)
					} else if (!isGroup) {
						replyca(`Untuk kontak penggunaan *${prefix}antidelete ctaktif*`)
					}
		   		} else if (argz[1] == 'ctaktif') {
					if (!isGroup) {
						if (isCtRevoke) return replyca(`Antidelete telah diaktifkan di semua kontak sebelumnya!`)
						dataCtRevoke.data = true
						fs.writeFileSync('./src/ct-revoked.json', JSON.stringify(dataCtRevoke, null, 2))
						replyca(`Antidelete diaktifkan disemua kontak!`)
					} else if (isGroup) {
						replyca(`Untuk grup penggunaan *${prefix}antidelete aktif*`)
					}
	   			} else if (argz[1] == 'banct') {
					if (isBanCtRevoke) return replyca(`kontak ini telah ada di database banlist!`)
					if (argz.length === 2 || argz[2].startsWith('0')) return replyca(`Masukan nomer diawali dengan 62! contoh 62838159xxxxx`)
					dataBanCtRevoke.push(argz[2] + '@s.whatsapp.net')
					fs.writeFileSync('./src/ct-revoked-banlist.json', JSON.stringify(dataBanCtRevoke, null, 2))
					replyca(`Kontak ${argz[2]} telah dimasukan ke banlist antidelete secara permanen!`)
	   			} else if (argz[1] == 'mati') {
					if (isGroup) {
						const index = dataRevoke.indexOf(from)
						dataRevoke.splice(index, 1)
						fs.writeFileSync('./src/gc-revoked.json', JSON.stringify(dataRevoke, null, 2))
						replyca(`*Succes disable Antidelete Grup!*`)
					} else if (!isGroup) {
						replyca(`Untuk kontak penggunaan *${prefix}antidelete ctmati*`)
					}
   				} else if (argz[1] == 'ctmati') {
					if (!isGroup) {
						dataCtRevoke.data = false
						fs.writeFileSync('./src/ct-revoked.json', JSON.stringify(dataCtRevoke, null, 2))
						replyca(`Antidelete dimatikan disemua kontak!`)
					} else if (isGroup) {
						replyca(`Untuk grup penggunaan *${prefix}antidelete mati*`)
					}
   				}
   				break
			   case 'nsfw':
            ...
            case 'asupan':
               ...
               nuy.sendMessage(from, asupan, video, {quoted: caa, mimetype: 'video/mp4', duration: 99999})
            ...
            case 'asupan2':
               ...
               randIndex = Math.floor(Math.random() * jsonData.length);
               randKey = jsonData[randIndex];
               asupan2 = await getBuffer(randKey.result)
               vidnya = await nuy.prepareMessage(from, asupan2, video)
               gbutsan = [
                 {buttonId: `${prefix}asupann2`, buttonText: {displayText: 'VIDEO ASUPAN'}, type: 1}
               ]
               buttonMessages = {
               ...
               await nuy.sendMessage(from, buttonMessages, MessageType.buttonsMessage, {contextInfo: {mentionedJid: [sender]}, quoted: ftrolii})
             	break
            case 'asupann2':
               if (isBanned) return replyca(mess.only.benned)
               if (!isUser) return replyca(mess.only.userB)
               ...
               randIndex = Math.floor(Math.random() * jsonData.length);
               randKey = jsonData[randIndex];
               asupann2 = await getBuffer(randKey.result)
               vidnya = await nuy.prepareMessage(from, asupann2, video)
               gbutsan = [
                 {buttonId: `${prefix}asupan2`, buttonText: {displayText: 'VIDEO ASUPAN'}, type: 1}
               ]
               buttonMessages = {
                 videoMessage: vidnya.message.videoMessage
                 contentText: `Hai ${pushname}\nVideo Asupan Done`,
            ...
            case 'fbutton':
               ...
               var xieeya = body.slice(8)
			      var namanya = xieeya.split("/")[0];
				   ...
				   encmedia = JSON.parse(JSON.stringify(caa).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo
			   	media = await nuy.downloadMediaMessage(encmedia)
				   punyaku = fs.writeFileSync('pinky.jpg', media)
            ...
            case 'infogempa':  
               ...
               if (isLimit(sender)) return replyca(limitend(pushname))
				...
				case 'setppbot':
				   ...
				   nuy.updatePresence(from, Presence.composing) 
					if (!isQuotedImage) return replyca(`Kirim gambar dengan caption ${prefix}setppbot atau tag gambar yang sudah dikirim`)
					...
					grupinv('Makasih profil barunya☺️')
					break
			   case 'google':
...
Giliran = @${tty.player1.split('@')[0]}`
  nuy.sendMessage(from, ucapan, text, {quoted: caa, contextInfo:{mentionedJid: [tty.player1,tty.player2]}})
  }
...
naa = ky_ttt.filter(toek => !toek.id.includes(from)) 
ky_ttt = naa
nuy.sendMessage(from, `Yahh @${tty.player2.split('@')[0]} Menolak Untuk Bermain Tictactoe`,text,{quoted:caa,contextInfo:{mentionedJid:[tty.player2]}})
}
}
if (isTTT && isPlayer1){
nuber = parseInt(budy)
if (isNaN(nuber)) return
if (nuber < 1 || nuber > 9) return replyca('Masukan Angka Dengan Benar')
main = ky_ttt.filter(hjh => hjh.id.includes(from)) 
if (!tttawal.includes(main[0].angka[nuber])) return replyca('Udah Di Isi, Isi Yang Lain Gan')
if (main[0].gilir.includes(sender)) return replyca('Tunggu Giliran Gan')
s = '❌'
...
${ttt}`
nuy.sendMessage(from, ucapan1, text, {quoted:caa, contextInfo:{mentionedJid: [tty.player1]}})
naa = ky_ttt.filter(hhg => !hhg.id.includes(from))
return ky_ttt = naa
}
...
${ttt}`
nuy.sendMessage(from, ucapan1, text, {quoted:caa, contextInfo:{mentionedJid: [tty.player2]}})
naa = ky_ttt.filter(hhg => !hhg.id.includes(from))
return ky_ttt = naa
}
