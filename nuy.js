...
async function starts() {
	...
	nuy.on('chat-update', async (caa) => {
		try {
         ...
         const fakeca = { key: { fromMe: false, participant:`0@s.whatsapp.net`, ...(from ? { remoteJid: "6283815956151-1624547970@g.us" }: {}) }, "message": {"extendedTextMessage": {"text": "Pinky X Manca", "previewType": "NONE", "contextInfo": {"stanzaId": "3EB0E2EE49D2", "participant": `0@s.whatsapp.net`, "quotedMessage": {"extendedTextMessage": {"text": "PinkyBot", "matchedText": "https://wa.me/6283893169044", "description": "Akun Private", "title": `Hai ${pushname}`, "previewType": "NONE", "jpegThumbnail": fs.readFileSync('pinky.jpg')}}}}}}
...
function addMetadata(packname, author) {	
	...
	fs.writeFile(`./${name}.exif`, buffer, (err) => {	
			...
			const checkLimit = (sender) => {
				...
				try {
					const getmemex = groupMembers.length
					    ...
					    nuy.sendMessage(from, `Maaf Pinky Tidak Bisa Masuk Ke Grup *${groupMetadata.subject}*\n\nMinimal Member Harus *${memberLimit}*`, text)
					setTimeout( () => {
                            nuy.groupLeave(from)
                            }, 100)
		setTimeout( () => {
		...
		setTimeout( () => {
		reply(`Pinky Bot Akan Segera Keluar Dari Group *${groupMetadata.subject}*`)
		}, 0)
		}
...
「 *PINKY BOT* 」`
          nuy.sendMessage(from, allmenu, MessageType.text, {quoted: fakeca, contextInfo: { forwardingScore: 99999, isForwarded: true}})
          break
          case 'grupmenu':
                if (isBanned) return fakeitem(mess.only.benned)
                if (!isUser) return fakeitem(mess.only.userB)
...
╰────────𖧹`
            ...
            case 'unmute':
               ...
               fakeitem('*Sukses*')
               console.log('Sukses unmute chat = ' + from)
               break
            case 'rules':
               nuy.sendMessage(from, rules(name), MessageType.text, {quoted: fakeca, contextInfo: { forwardingScore: 99999, isForwarded: true}})
    	         break
            case 'return':
				case '>':
				   if (!isOwner) return fakeitem(mess.only.ownerB)
            ...
            case 'takestick':
				case 'take':
						if (!isQuotedSticker) return fakeitem(`Reply sticker dengan caption *${prefix}takestick nama/author*`)
						const pembawm = body.slice(11)
						if (!pembawm.includes('/')) return await fakeitem(`Reply sticker dengan caption *${prefix}takestick nama/author*`)
						const encmedia = JSON.parse(JSON.stringify(caa).replace('quotedM','m')).message.extendedTextMessage.contextInfo
						const media = await nuy.downloadAndSaveMediaMessage(encmedia, `./${sender}.webp`)
						const packname = pembawm.split('/')[0]
						const author = pembawm.split('/')[1]
						exif.create(packname, author, `./${sender}.webp`)
						exec(`webpmux -set exif ./${sender}.exif ./${sender}.webp -o ./${sender}.webp`, async (error) => {
							if (error) return fakeitem('Eror')
							nuy.sendMessage(from, fs.readFileSync(`./${sender}.webp`), sticker, {quoted: caa})
							fs.unlinkSync(media)
							fs.unlinkSync(`./${sender}.exif`)
						})
						break
// makermenu
            case 'darkneon':
...
//creator
				...
				case 'nulis2':
				   ...
				   if (!isUser) return fakeitem(mess.only.userB)
					if (args.length < 1) return fakeitem(`${name} Harus Nulis Apa Kak??`)
				   ct = body.slice(8)
				   ct = await getBuffer(`http://lolhuman.herokuapp.com/api/nulis?apikey=${ZeksApi}&text=${ct}`)
